...
## 3. isinstance()
“Is this value already a True or False?”
- Why it’s used
...
## 4. Common “is-something” checks you’ll see a lot
- isinstance(x, type)
...
```
Key idea
to_bool → the function itself
to_bool() → running the function immediately gets the result
...
for value in df["paid"]:
    to_bool(value)
```
So pandas supplies the value — you don’t.


## What `np.where()` is
`np.where()` is a NumPy function used to **choose values based on a condition**.  
Think of it as a **vectorized if–else** that works on arrays or pandas Series.

### Basic form
```python
np.where(condition, value_if_true, value_if_false)
condition → array-like of True / False
```
value_if_true → value to use where condition is True
value_if_false → value to use where condition is False
